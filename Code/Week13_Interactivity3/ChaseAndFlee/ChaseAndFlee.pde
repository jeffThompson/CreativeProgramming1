...
While many books have been written on interactive and game 
artificial intelligence, this example shows a VERY basic chase
and flee behavior.
...
...and Daniel Shiffman's excellent book "Nature of Code" for tons
of really great examples.
...
float chaseSpeed = 2.0;       // speed to chase and flee
float fleeSpeed =  0.5;       // (try changing and see what happens)

PVector player, chase, flee;  // positions for the 3 elements
...
void draw() {
  ...
  // the chasing ball checks which direction the player is in,
  // then moves incrementally that direction
  PVector dir = PVector.sub(player, chase);  // PVector makes this math easy!
  ...
  // here we also only flee if the player is within 100px,
  // which gives a more natural interaction
