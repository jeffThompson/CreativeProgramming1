...
FINITE STATE MACHINE AI
 ...
 object can only have one state at a time, and those states determine
 what it does. With well-chosen states, we can create complex-seeming
 behavior that can mimic physical conditions (how the entity moves) or
 even emotional states (angry, curious, idle).
 
 For example, in this example we'll make a "zombie" (well, a circle that
 has zombie-like behavior), but you can substitute the "attack" langauge
 for any other kind of behavior. 
 
 Here are some common game AI states:
 ...
 some if/else statements to test the conditions and trigger the behaviors,
 and the PVector class to make the movement math more simple.
 ...
 CHALLENGE:
 +  Can you have the "flee" state be triggered when the player attacks,
    maybe by clicking or hitting the spacebar?
 +  Can you give the zombie a better "idle" state? Hint: try sin() or
    Perlin noise to make it move.
...
PVector player, zombie;               // positions, created in setup()
...
void setup() {
  ...
  // initial positions (use the mouse to move your player)
